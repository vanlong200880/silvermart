	...
	***/
	...
	function um_submit_form_errors_hook_login( $args ){
		...
		if ( isset( $args['username'] ) && $args['username'] == '' ) {
			$ultimatemember->form->add_error( 'username',  __('Nhập tên đăng nhập hoặc email','ultimatemember') );
		}

		if ( isset( $args['user_login'] ) && $args['user_login'] == '' ) {
			$ultimatemember->form->add_error( 'user_login',  __('Nhập tên đăng nhập','ultimatemember') );
		}

		if ( isset( $args['user_email'] ) && $args['user_email'] == '' ) {
			$ultimatemember->form->add_error( 'user_email',  __('Nhập địa chỉ Email','ultimatemember') );
		}
		...
		if ( !username_exists( $user_name ) ) {
			if ( $is_email ) {
				$ultimatemember->form->add_error( $field,  __(' Chúng tôi không thể tìm thấy Email của bạn.','ultimatemember') );
			} else {
				$ultimatemember->form->add_error( $field,  __(' Chúng tôi không thể tìm thấy tên đăng nhập của bạn.','ultimatemember') );
			}
		} else {
			if ( $args['user_password'] == '' ) {
				$ultimatemember->form->add_error( 'user_password',  __('Nhập mật khẩu','ultimatemember') );
			}
		}
		...
		if ( $user && wp_check_password( $args['user_password'], $user->data->user_pass, $user->ID) ) {
			...
			$ultimatemember->form->add_error( 'user_password',  __('Mật khẩu không đúng.','ultimatemember') );
		}
	...
	***/
	...
	?>
		...
		<div class="um-col-alt-b">
			<a href="<?php echo um_get_core_page('password-reset'); ?>" class="um-link-alt"><?php _e('Quên mật khẩu?','ultimatemember'); ?></a>
		</div>
