			...
			foreach( $fields as $key => $array ) {
				...
				if ( isset( $args[$key] ) ) {
					...
					if ( isset( $array['force_confirm_pass'] ) && $array['force_confirm_pass'] == 1 ) {
						...
						$ultimatemember->form->add_error( 'confirm_' . $key , __('Nhập lại mật khẩu','ultimatemember') );
						}
						...
						switch( $array['validate'] ) {
							...
							case 'numeric':
								if ( $args[$key] && !is_numeric( $args[$key] ) ) {
									$ultimatemember->form->add_error($key, __('Dữ liệu phải là số.','ultimatemember') );
								}
								break;

							case 'phone_number':
								if ( !$ultimatemember->validation->is_phone_number( $args[$key] ) ) {
									$ultimatemember->form->add_error($key, __('Nhập số điện thoại','ultimatemember') );
								}
								break;
							...
							case 'unique_username':
								...
								if ( $args[$key] == '' ) {
									$ultimatemember->form->add_error($key, __('Nhập tên đăng nhập','ultimatemember') );
								} else if ( $mode == 'register' && username_exists( sanitize_user( $args[$key] ) ) ) {
									$ultimatemember->form->add_error($key, __('Tên đăng nhập đã tồn tại.','ultimatemember') );
								} else if ( is_email( $args[$key] ) ) {
									$ultimatemember->form->add_error($key, __('Tên đăng nhập hoặc Email không được sử dụng.','ultimatemember') );
								} else if ( !$ultimatemember->validation->safe_username( $args[$key] ) ) {
									$ultimatemember->form->add_error($key, __('Tên đăng nhập không đúng.','ultimatemember') );
								}
							...
							case 'unique_username_or_email':
								...
								if ( $args[$key] == '' ) {
									$ultimatemember->form->add_error($key,  __('Email không được sử dụng.','ultimatemember') );
								} else if ( $mode == 'register' && username_exists( sanitize_user( $args[$key] ) ) ) {
									$ultimatemember->form->add_error($key, __('Email đã tồn tại','ultimatemember') );
								} else if ( $mode == 'register' && email_exists( $args[$key] ) ) {
									$ultimatemember->form->add_error($key,  __('Email đã được sửa dụng.','ultimatemember') );
								} else if ( !$ultimatemember->validation->safe_username( $args[$key] ) ) {
									$ultimatemember->form->add_error($key,  __('Email không đúng.','ultimatemember') );
								}
							...
							case 'unique_email':
								...
								if ( in_array( $key, array('user_email') ) ) {
									...
									if ( $args[$key] != '' && !is_email($args[$key]) ) {
										$ultimatemember->form->add_error($key, __('Email không tồn tại.','ultimatemember') );
									} else if ( $args[$key] != '' && email_exists( $args[$key] ) ) {
										$ultimatemember->form->add_error($key, __('Email đã được sử dụng.','ultimatemember') );
									} else if ( $args[$key] != '' ) {
