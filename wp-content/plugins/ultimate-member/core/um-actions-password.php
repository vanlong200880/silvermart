	...
	***/
	...
	function um_reset_password_errors_hook( $args ) {
		...
		if ( $live_timestamp - $form_timestamp < 3 && um_get_option('enable_timebot') == 1 )
			wp_die( __('Bạn đã thực hiện gửi thông tin quá 3 lần. Chúng tôi sẽ không đáp ứng yêu cầu của bạn. Bạn vui lòng thử lại sau. Cảm ơn!') );

		if ( strlen(trim( $_POST['username_b'] ) ) == 0 ) {
			$ultimatemember->form->add_error('username_b', __('Tên đăng nhập hoặc Email không đúng','ultimatemember') );
		}

		$user = $_POST['username_b'];

		if ( ( !is_email( $user ) && !username_exists( $user ) ) || ( is_email( $user ) && !email_exists( $user ) ) ) {
			$ultimatemember->form->add_error('username_b', __('Chúng tôi không thể tìm thấy email hoặc tên đăng nhập của bạn.','ultimatemember') );
		} else {
	...
	***/
	...
	function um_reset_password_form($args) {
		...
		<div class="um-col-alt um-col-alt-b">
			...
			<div class="um-center"><input type="submit" value="<?php _e('Lấy lại  mật khẩu','ultimatemember'); ?>" class="um-button" /></div>
