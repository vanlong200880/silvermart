...
(function ($) {
	...
	MbComingsoon = function (date, element, localization, speed, callBack, gmt, showText) {
		...
		if (jQuery.isFunction(callBack))
			...
			this.callBack = null;
		this.localization = {days: "Ngày", hours: "Giờ", minutes: "Phút", seconds: "Giây"};
	...
	MbComingsoon.prototype = {
		...
		updateCounter    : function () {
			this.updatePart('Ngày');
			this.updatePart('Giờ');
			this.updatePart('Phút');
			this.updatePart('Giây');
			if (this.timeOut()) {
				this.active = false;
				if (this.callBack)
					this.callBack(this);
	...
	jQuery.fn.mbComingsoon = function (opt) {
		var defaults = {
			interval    : 1000,
			callBack    : null,
			localization: { days: "Ngày", hours: "Giờ", minutes: "Phút", seconds: "Giây" },
			speed       : 500,
			gmt         : 0,
			showText    : 1
		}
